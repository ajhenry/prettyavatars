...
function generateData(name, colors) {
    ...
    var wrapperTranslateX = preTranslateX < 5 ? preTranslateX + SIZE / 9 : preTranslateX;
    var preTranslateY = getUnit(numFromName, 10, 2);
    var wrapperTranslateY = preTranslateY < 5 ? preTranslateY + SIZE / 9 : preTranslateY;
    var data = {
...
var AvatarBeam = function (props) {
    var data = generateData(props.name, props.colors);
    return (<svg viewBox={'0 0 ' + SIZE + ' ' + SIZE} fill='none' role='img' xmlns='http://www.w3.org/2000/svg' width={props.size} height={props.size}>
      ...
      <g mask='url(#mask__beam)'>
        ...
        <rect x='0' y='0' width={SIZE} height={SIZE} transform={'translate(' +
            ...
            ') scale(' +
            data.wrapperScale +
            ')'} fill={data.wrapperColor} rx={data.isCircle ? SIZE : SIZE / 6}/>
        <g transform={'translate(' +
            ...
            SIZE / 2 +
            ')'}>
          {data.isMouthOpen ? (<path d={'M15 ' + (19 + data.mouthSpread) + 'c2 1 4 1 6 0'} stroke={data.faceColor} fill='none' strokeLinecap='round'/>) : (<path d={'M13,' + (19 + data.mouthSpread) + ' a1,0.75 0 0,0 10,0'} fill={data.faceColor}/>)}
